...
const Block_11 = ({image,sloganPrincipal,title,text,listsAbout,listsServices}) =>{
    ...
    return(
        <section className="w-full relative min-h-screen h-auto mb-20 md:my-16 bg-cover bg-no-repeat bg-center flex justify-end"
        style={{ backgroundImage: `url("${image ? image : imgDefault}")` }}>

            <section className="absolute -top-10 bg-[#3333339d]  p-5 left-5 md:left-40 z-20 text-white">
                <h3>A Little About Us</h3>
            </section>
        
            <section className="w-full lg:w-[55%] min-h-screen trapecio bg-[#29292991]  lg:text-right pt-20 p-10 text-white">
                <section className="w-full md:w-[85%] h-auto mx-auto lg:mr-4">
                {
                            sloganPrincipal ?
                                rpdata?.dbPrincipal?.licensed ?
                                    <h2 className='p-3 capitalize text-white'>
                                        {rpdata?.dbPrincipal?.licensed}
                                    </h2>
                                    : <h3 className='pb-10 capitalize text-white'>
                                        {
                                ...
                                :
                                <h2 className='pb-3 capitalize text-white'>
                                    {
                                        title ? title :
